...
if (isset($_POST['addScript'])) {
  ...
  $download = $_POST['link'];
      ...
      $query = "INSERT INTO produtos(nome,description,version,valor,vendas,video,arquivo) VALUES ('$nome','$desc','$version','$valor','0','$video','$download');";
      $result = mysqli_query($conexao,$query);
      if ($result) {
        header("Refresh:0");
      } 
...
<body>
  ...
  <div class="stats">
        ...
        while($row = mysqli_fetch_array($query)){
          ?>
                  <tr>
                    ...
                    <td><a href="manage.php?editProduct=<?php echo $row['id'] ?>"><i class="fas fa-edit"></i></a> <a href="manage.php?delete=<?php echo $row['id'] ?>"><i class="fas fa-trash"></i></a></td>
                  </tr>
                  <?php } ?>
                </tbody>
              </table>
    ...
    <div class="col-md-4">
      <h4 class="titled">Adicionar Script</h4>
      <div class="addlicense">
        <form action="" method="POST" enctype="multipart/form-data">
          <input type="text" name="nome" placeholder="Nome do script">
          <textarea name="desc" placeholder="Descrição do script"></textarea>
          <input type="text" name="video" placeholder="ID Vídeo do script">
          <input type="text" name="version" placeholder="Versão inicial do script">
          <input type="number" name="valor" placeholder="Valor do Script">
          <br>
          <input type="text" name="link" placeholder="Download do Script">
          <button type="submit" name="addScript" class="confirm btn btn-success">Adicionar</button>
        </form>
      </div>
    </div>
    </div>
